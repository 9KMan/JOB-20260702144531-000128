...
The repo is split into two planes that share the same physical
database:

- **`src/`** — the *data layer*: Alembic env wiring, CLI smoke
  tests, async engine + session factory, and SQLAlchemy models.
  Does not import FastAPI.
- **`app/`** — the *request/response plane*: FastAPI routers,
  Pydantic schemas, structlog + OpenTelemetry, and the canonical
  orchestrator modules (pure Python, no HTTP).

Auxiliary directories:

- `alembic/` — autogenerated DB migration scripts
...
Top-level files: `pyproject.toml`, `requirements.txt`,
`docker-compose.yml`, `Dockerfile`, `.env.example`, `SPEC.md`,
`OUT_OF_SCOPE.md`, `README.md`.
